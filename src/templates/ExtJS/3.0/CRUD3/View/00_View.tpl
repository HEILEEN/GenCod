...
<head>
	...
	<!--	/*Ajuste de tamaño de texto con el tamaño de cada fila*/ -->
	<style type="text/css">
			...
			initUIGrid: function(){
				grid = new Ext.grid.GridPanel({
					...
					tbar: [{
						    ...
						    {
								text:'Exportar', icon: '../../../rc/img/icons/fam/page_excel.png', 
							...
							{ 
								...
								iconCls: 'icon-help',
								scope: this,
								handler: function(){ mainWindow.showHelp({ modu: MODU_XWIKI_URL });} 
							}/*Fin Ayudas*/	],
					...
					plugins:[
						new Ext.ux.grid.GridFilters({
							...
							filters:[
								...
								{type: 'date',  dataIndex: '$c.loName$', dateFormat:'Y/m/d', afterText: 'Despues', beforeText : 'Antes', onText: 'En'}
								$else$
...
{c|$if(!c.type.isFK)$$if(c.type.integer)$
			...
			initEventEventHandlers: function(){
				...
				wndEditForm.on('beforehide', function(el){ 
					this.onBeforehide(el); 
				}, this);
			},
			...
			/*
			 * Evento que se ejecuta antes de ocultar
			 */ 
			onBeforehide: function(el){
				dsGrid.load();
			},
			...
			/*
			 * Evento exportar registros
			 */
			onExportar: function(){
				
				var params = new Object();
				
				if(dsGrid.sortInfo!=null)
				{														
					params ={
								app: "$AppName$", 
			...
			onSave: function(){
				...
				{
					//Ext.App.msg('Aviso', 'No ahi datos para guardar');
				}
			},
